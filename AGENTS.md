...
`dcc-mcp-maya` embeds a standards-compliant MCP Streamable HTTP server directly inside Autodesk Maya. It exposes 206 Maya operations as MCP tools that any AI agent (Claude, Cursor, Gemini, etc.) can call over HTTP — no external gateway, no subprocess bridge.

**Current version:** 0.8.15 <!-- x-release-please-version -->
**Core dependency:** `dcc-mcp-core>=0.18.36,<1.0.0`
**Python:** 3.7+
**Maya:** 2020+
...
A: `src/dcc_mcp_maya/skills/` (27 packages, 205 scripts, 206 MCP tools). Each package contains `SKILL.md`, `tools.yaml`, optional `groups.yaml`, and `scripts/*.py`.
...
| `src/dcc_mcp_maya/skills/` | 27 built-in skill packages, 205 scripts |
