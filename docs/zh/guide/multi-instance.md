...
跨机器高可用部署见
[`dcc-mcp-core` 生产部署指南](https://github.com/dcc-mcp/dcc-mcp-core)
(core #330)。本文只覆盖单机场景。
...
2. **所有实例必须共用同一个 `FileRegistry` 目录**。Sidecar 模式下，
   第一个发现网关缺失的 Maya sidecar 会确保单机 gateway 启动；所有 Maya
   进程都作为普通 backend 注册。不同的 `DCC_MCP_REGISTRY_DIR` 会让它们
...
4. **网关独立于任何一个 Maya 进程**。某个 Maya 退出后，其 sidecar
   同步退出，gateway 会清理该 backend；其他 DCC 实例继续可用。

## 启动 N 个实例

仓库里的 `examples/multi-instance/userSetup.py` 把实例端口交给 core/OS
分配，并用 `dcc_pid=os.getpid()` 注册。把它放到 Maya `scripts/` 目录，
每个 Maya 下次启动时都会自动注册，且没有“先探测、再绑定”的端口竞争：
...
│  workstation-01                                                │
│                                                                │
│  Maya 2025.0（动画）    port=<OS A>  dcc_pid=1234  minimal=1  │
│  Maya 2025.1（Lookdev） port=<OS B>  dcc_pid=5678  minimal=0  │
│  Maya 2025.2（批量）    port=<OS C>  dcc_pid=9012  minimal=1  │
│                              │                                 │
│                              ▼                                 │
│  共享 FileRegistry ──► 稳定网关 :9765                         │
└────────────────────────────────┬───────────────────────────────┘
                                 │
                                 ▼
                           MCP 客户端
...
Sidecar 模式下 Maya 实例本来就不承担 gateway 角色；它们确保独立 gateway
运行后作为 backend 注册。这是设计预期。若需要单独的路由域，请为 launcher
配置独立的 `DCC_MCP_GATEWAY_PORT` 和匹配的 `DCC_MCP_REGISTRY_DIR`。
...
- [`examples/multi-instance/`](https://github.com/dcc-mcp/dcc-mcp-maya/tree/main/examples/multi-instance) — 可运行的 `userSetup.py`。
- Issue [#88](https://github.com/dcc-mcp/dcc-mcp-maya/issues/88) —
  本文的验收标准源头。
