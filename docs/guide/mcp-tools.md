...
Once the `dcc-mcp-maya` server is running inside Maya, your AI assistant can discover **24 Maya skill packages** and load **72+ typed Maya tools** on demand. Just describe what you want in plain English; the assistant should search capabilities, load the matching skill, then call the typed tool.
