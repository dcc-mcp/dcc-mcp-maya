...
def _skin_rows_match(cmds, cluster, mesh, influences, snapshots):
    return all(_read_row(cmds, cluster, mesh, vertex, influences) == values for vertex, values in snapshots.items())
...
def set_skin_weights(
    ...
    try:
        ...
        before_by_vertex = {vertex: _read_row(cmds, cluster, mesh, vertex, influences) for vertex, _pairs in validated}
        transaction = MayaUndoChunk(cmds, "dcc_mcp_set_skin_weights")
        transaction.begin()
            ...
            for influence, value in zip(influences, values):
                if abs(value - expected.get(influence, 0.0)) > tolerance:
                    receipt = transaction.rollback(
                        lambda: _skin_rows_match(cmds, cluster, mesh, influences, before_by_vertex)
                    )
                    return skill_error(
        ...
        receipt = {}
        if transaction is not None and before_by_vertex is not None:
            receipt = transaction.rollback(lambda: _skin_rows_match(cmds, cluster, mesh, influences, before_by_vertex))
        return skill_exception(exc, message="Failed to set skin weights", **receipt)
