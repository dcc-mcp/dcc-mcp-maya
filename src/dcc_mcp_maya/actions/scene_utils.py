...
_ACTIONS = [
    ...
    ('create_polygon_text', 'Create 3D polygon text in the scene', 'geometry', ['text', 'polygon', '3d', 'create']),
    ...
    ('get_object_matrix', 'Get the world or local transformation matrix of a node', 'utility', ['matrix', 'transform', 'query']),
    ('set_object_matrix', "Set a node's transform from a 4x4 world matrix", 'utility', ['matrix', 'transform', 'set']),
    ('reset_transform', 'Reset translate/rotate/scale channels to identity', 'utility', ['transform', 'reset', 'identity']),
    ('set_transform_space', 'Set translate/rotate of an object in world/local/parent/object space', 'utility', ['transform', 'space', 'world', 'local']),
]
def set_pivot(
    object_name: str,
