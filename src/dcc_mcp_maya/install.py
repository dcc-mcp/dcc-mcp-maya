    ...
    with archive:
        ...
        for info in files:
            ...
            if not relative_parts:
                continue
            normalized["/".join(relative_parts)] = info.filename
    required = {
...
def _receipt_payload(ctx, module_stage, descriptor_stage, user_setup_stage, previous_receipt, prior_user_setup):
    if previous_receipt and isinstance(previous_receipt.get("previous_user_setup"), dict):
        previous_user_setup = previous_receipt["previous_user_setup"]
    else:
        previous_user_setup = {
            "existed": ctx.user_setup_path.is_file(),
            "content_base64": base64.b64encode(prior_user_setup).decode("ascii"),
            "sha256": hashlib.sha256(prior_user_setup).hexdigest(),
        }
