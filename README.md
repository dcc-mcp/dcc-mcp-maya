...
- **Verified output:** A cold Maya 2026 / Arnold 7.4.3.2 batch render of `DCC_MCP_Real_Honeybee_AssetSync_v2.mb` retains **61 native Maya joints**, **1,278 editable animation curves**, **78 materials**, and **32,231 editable NURBS groom/guide curves**. The Houdini rig is carried as a standard 60-joint `UsdSkel` hierarchy; Maya reconstructs the extra skeleton root and uses **75 reversible rigid-part bindings** so joint edits drive the native mesh and groom hierarchy without inventing soft-skin weights. Two tapered Curve Collectors use PBR `aiStandardHair`, geometry uses editable metal/rough `standardSurface` assignments, and all four wings retain synchronized membrane/vein keys across frames 1–96.
