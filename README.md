...
[![dcc-mcp-core](https://img.shields.io/badge/dcc--mcp--core-%3E%3D0.19.45%2C%3C1.0.0-blue)](https://github.com/dcc-mcp/dcc-mcp-core)
...
- **Editable transfer:** The Maya scene retains **61 native joints**, **946 editable animation curves**, and **31,306 native NURBS groom/vein curves**. Arnold curve collectors add editable root-to-tip width profiles without replacing the source curves with mesh tubes. Six project-relative 2K texture files drive sRGB Base Color plus Raw roughness/height through native UVs and editable `standardSurface`/`aiStandardHair` nodes. AssetSync inspection reports image paths, color spaces, existence, and their bounded material-input connections, including utility-node hops.
- **Verified Arnold output:** Maya 2026 / Arnold 7.4.3.2 cold renders at frames 1, 17, 33, 49, 65, and 81 produce six distinct joint-pose hashes. The scene uses ACEScg rendering with the ACES 1.0 SDR-video view, a camera-hidden CC0 [Cloudy Vondelpark HDRI](https://polyhaven.com/a/cloudy_vondelpark), and neutral softboxes. The technical chart changes by at most **1/255** across light variants while all three physical reference spheres respond to the rig. The decoded GIF uses one palette with 24 chart and three reference entries reserved, no dithering, **0/255** temporal chart drift, and at most **2/255** source error.
