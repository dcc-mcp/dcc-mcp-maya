...
- **Verified output:** A cold Maya 2026 / Arnold 7.4.3.2 batch render of the AssetSync v2 revision-7 scene retains **61 native Maya joints**, **946 editable joint animation curves**, **105 Arnold `standardSurface` materials**, and **31,306 editable NURBS groom/guide curves**. Maya uses **75 reversible rigid-part bindings** so joint edits drive the native mesh and groom hierarchy without inventing soft-skin weights. The groom remains editable native curves with `aiStandardHair`; chitin and gold materials have connected roughness and micro-bump inputs; wings use a thin-walled dielectric material. A neutral camera-visible background is decoupled from HDRI lighting, three white softboxes expose PBR response, and the technical chart remains light-invariant through the active ACEScg → ACES SDR view. All four wings retain synchronized membrane/vein joint drivers across frames 1–96.
